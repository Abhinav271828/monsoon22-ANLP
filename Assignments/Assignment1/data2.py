    ...
    def __init__(self, file_path           : str               = 'data/train.txt',
        ...
        with open(self.file_path, 'r') as f:
            for line in tqdm(f, desc="Creating dataset"):
                ...
                contexts = torch.tensor(indices + [0]*(self.max_length - len(indices)))
                targets = torch.tensor(indices[1:] + [-100] * (self.max_length-len(words) + 1))
                if (len(targets) > self.max_length): print(line)

...
class RNN(nn.Module):
    ...
    def forward(self, init_hidden_state, inputs):
        ...
        for _ in range(self.n):
            ...
            #            [seq, bz, is]
            for input in inputs.transpose(0,1):
...
class LanguageModel(nn.Module):
    ...
    def train_epoch(self, dl, optimiser, loss_fn):
        for batch in tqdm(dl):
            ...
            logits = self.forward(torch.zeros(contexts.shape[0], self.rnn.cell.hs).to(device), contexts)
...
perp_file('../../data/train.txt', '2020114001-LM2-train.txt', train_ds)
perp_file('../../data/test.txt', '2020114001-LM2-test.txt', train_ds)
